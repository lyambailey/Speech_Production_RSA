...
# LSA on data from the Test Phase of the 'PE' experiment. These files are
...
origTemplateFn = join(assetsPath, 'FSL_templates', 'PE_test_template_firstLevel.fsf')
...
for subject in subjects:
    ...
    foil_fn = join(subjectInDir, 'foil_words.txt')

    # Read in each word list (in alphabetical order) and join together
    ...
    with open(foil_fn) as f:
        foil_words = sorted(f.read().split("\n"))
        foil_words = list(filter(None, foil_words))   # Removes empty list entry

    words = sorted(aloud_words + silent_words + foil_words)
    ...
    for run in runs:
        ...
        # # run FEAT - NOTE, this is now handled by a different script, which
        # runs FEAT analyses with a parallel function to improve efficiency.

        # print('Starting', subject, run)
        # os.system('feat ' + outputDesignFn)
