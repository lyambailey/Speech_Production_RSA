...
% %% For development purposes only: contrain the data to a single ROI.
% 
% ROIs_path = fullfile(top_dir, 'MRIanalyses', 'assets', 'Harvard_Oxford_ROIs');
% 
% % Load MNI template and mask out cerebellum
% MNI_mask_fn = fullfile(top_dir, 'MRIanalyses', 'assets', 'MNI152_T1_2mm_brain_mask.nii.gz');
% MNI_template = cosmo_fmri_dataset(MNI_mask_fn);
% 
% cerebellum_mask_fn = fullfile(ROIs_path, 'Cerebellum.nii.gz');
...
% 
% AG_ROI = cosmo_fmri_dataset(fullfile(ROIs_path, 'Angular_Gyrus_LH.nii.gz'), 'mask', MNI_template_masked);
% 
% % Finally, use the ROI to mask the group data. Jesus wept.
% ds_group = cosmo_slice(ds_group, AG_ROI.samples ~= 0, 2);
% 
% % Remove useless samples from the stacked data
...
searchlight_measure = @compute_between_subjects_tarnsformation_iter_fullybetween;
...
for i_cond=1:numel(conditions)
    ...
    for i_subj=1:numel(subjects)
        ...
        cosmo_map2fmri(average_searchlight_results, ...
            fullfile(out_path, sprintf('%s_%s_study-test_between_subjects_transformation_effsize_fullybetween.nii.gz', subject_id, condition)));
        toc
    end % subjects
end % conditions 
